...
module FingerTree.Internal where
...
measureDigitL = \case
  ...
  ThreeL x y z -> measure x <> measure y <> measure z
  FourL w x y z -> measure w <> measure x <> measure y <> measure z
...
data FingerTree
   ...
   | FingerTree_Deep {-# UNPACK #-} !Measure DigitL (DeepTree 'Level_Leaf) DigitL
...
data Place a = Place {-# UNPACK #-} !Measure a

-- | If 'Measure' is ordered and that order is compatible with its monoid structure
-- then we can look up the element at the point in the 'FingerTree' where the measure
-- exceeds some threshold.
--
-- If 'Measure' is furthermore a group then we can do so more efficiently.
lookupOrdG :: (Ord Measure, Group Measure) => Measure -> FingerTree -> Maybe Elem
lookupOrdG !i
  | i < mempty = const Nothing
  | otherwise = \case
    FingerTree_Empty -> Nothing
    ...
    FingerTree_Deep m pr t sf -> case i < m of
      ...
      True
        | i < mpr
        -> Just $! lookupDigitLOrdG i pr
        | i < mt
        -> case lookupDeepTreeOrdG (i ~~ mpr) t of
             Place i' xs -> case lookupNodeLOrdG i' xs of
               Place _ x -> Just x
        | otherwise
        -> Just $! lookupDigitLOrdG (i ~~ mt) sf
     where
      mpr = measureDigitL pr
      mt = mpr <> measureDeepTree t
...
-- Precondition: The index is definitely smaller than the measure of the tree + prefix
lookupDeepTreeOrdG :: (Ord Measure, Group Measure) => Measure -> DeepTree l -> Place (Node l)
lookupDeepTreeOrdG !i = \case
  DeepTree_Empty -> illegal_argument "lookupDeepTree"
  DeepTree_Single n -> Place i n
  DeepTree_Deep _ pr t sf
    | i < mpr
    -> lookupDigitNOrdG i pr
    | i < mt
    -> case lookupDeepTreeOrdG (i ~~ mpr) t of
         Place i' xs -> lookupNodeNOrdG i' xs
    | otherwise
    -> lookupDigitNOrdG (i ~~ mt) sf
   where
    !mpr = measureDigitN pr
    mt = mpr <> measureDeepTree t

-- Precondition: The index is definitely smaller than the measure of the digit + prefix
lookupDigitNOrdG :: (Ord Measure, Group Measure) => Measure -> DigitN l -> Place (Node l)
lookupDigitNOrdG !i = \case
  OneN x -> Place i x
  TwoN x y
    | i < mx -> Place i x
    | otherwise -> Place (i ~~ mx) y
   where
    mx = measureNode x
  ThreeN x y z
    | i < mx -> Place i x
    | i < my -> Place (i ~~ mx) y
    | otherwise -> Place (i ~~ my) z
   where
    mx = measureNode x
    my = mx <> measureNode y
  FourN w x y z
...
-- Precondition: The index is definitely smaller than the measure of the node + prefix
lookupNodeLOrdG :: (Ord Measure, Group Measure) => Measure -> Node 'Level_Leaf -> Place Elem
lookupNodeLOrdG !i = \case
  Node_Leaf2 _ x y
    | i < mx -> Place i x
    | otherwise -> Place (i ~~ mx) y
   where
...
lookupNodeNOrdG :: (Ord Measure, Group Measure) => Measure -> Node ('Level_Branch l) -> Place (Node l)
lookupNodeNOrdG !i = \case
  Node_Branch2 _ x y
    | i < mx -> Place i x
    | otherwise -> Place (i ~~ mx) y
   where
